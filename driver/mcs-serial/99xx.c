...
static _INLINE_ void transmit_chars(struct uart_99xx_port *up)
{
	...
	DEBUG("In %s ---------------------------------------START\n",__FUNCTION__);
	if (up->port.x_char) {
		DEBUG("X_CHAR is there\n");
		serial_out(up, UART_TX, up->port.x_char);
		up->port.icount.tx++;
		up->port.x_char = 0;
		return;
	}
	DEBUG("xmit->head is %d and xmit->tail is %d\n",xmit->head,xmit->tail);
...
int serial99xx_register_port(struct uart_port *port,struct pci_dev *dev)
{
...
static int  serial99xx_probe(struct pci_dev *dev, const struct pci_device_id *ent)
{
	...
	retval = serial99xx_register_port(&serial_port,dev);
	if (retval < 0){
		DEBUG("Couldn't register serial port %s, retval=%d: \n", pci_name(dev),retval);
		goto disable;	
	}	

	//Register a ISR
	#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
	if ((retval = request_irq(dev->irq, serial99xx_interrupt,SA_SHIRQ,"saturn",&serial99xx_ports[retval]))) 
		  goto disable;
	#else
	if ((retval = request_irq(dev->irq, serial99xx_interrupt,IRQF_SHARED,"pcie-msi-serial",&serial99xx_ports[retval]))) {
